            ...
            <div className="relative bg-gradient-to-br from-primary-50 via-accent-50 to-orange-50 dark:from-gray-900 dark:via-gray-900 dark:to-gray-900 overflow-hidden border-b border-primary-100 dark:border-gray-800">
                ...
                <div className="w-full xl:mx-auto xl:max-w-[1440px] px-4 xl:px-0 py-10 md:py-14 relative z-10">
                    <nav className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-5">
                        <Link href="/" className="hover:text-primary-600 transition-colors font-medium">Trang chủ</Link>
                        <i className='bx bx-chevron-right text-xs' />
                        <span className="text-primary-600 dark:text-primary-400 font-semibold">Hỗ trợ khách hàng</span>
                    </nav>
                    <h1 className="text-3xl md:text-4xl font-bold text-gray-800 dark:text-gray-100 mb-3">
                        Hỗ trợ <span className="text-primary-600 dark:text-primary-400">khách hàng</span>
                    </h1>
                    ...
                    <div className="flex flex-col lg:flex-row gap-8">
                        ...
                        <aside className="lg:w-72 shrink-0">
                            <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden sticky top-24">
                                <div className="bg-cta-600 px-5 py-4">
                                    ...
                                    {sidebarLinks.map(link => (
                                        <Link key={link.href} href={link.href}
                                            className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm transition-all duration-200 ${pathname === link.href
                                                ? 'bg-primary-50 dark:bg-primary-950/40 text-primary-700 dark:text-primary-400 font-semibold border border-primary-200 dark:border-primary-800/50'
                                            ...
                                            <i className={`bx ${link.icon} text-lg ${pathname === link.href ? 'text-primary-500 dark:text-primary-400' : 'text-gray-400 dark:text-gray-500'}`} />
                                            {link.label}
                                        </Link>
                                    ))}
                                </nav>
