...
const ChatWindow = ({ onClose }) =>
{
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="flex min-h-[3.1rem] items-center justify-between bg-cta-600 px-[1.1rem] py-[0.85rem] text-white shadow-[0_2px_8px_rgba(245,158,11,0.18)]">
            ...
            return (
              <div key={msgId} className={`flex ${isFromCurrentUser ? 'justify-end' : 'justify-start'}`}>
                <div className="flex max-w-[80%] items-end gap-[0.4rem]">
                  {!isFromCurrentUser && (
                    <div className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full bg-cta-600 text-[0.7rem] font-bold text-white">A</div>
                  )}
                  <div className={`px-3 py-2 text-[0.9rem] leading-[1.4] shadow-sm ${
                    isFromCurrentUser
                      ? 'rounded-[0.8rem_0.8rem_0.2rem_0.8rem] bg-cta-600 text-white shadow-primary-500/15'
