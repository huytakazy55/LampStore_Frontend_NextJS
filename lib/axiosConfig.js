...
import { getApiErrorMessage } from './apiErrorHelper';
        ...
        if (response) {
            ...
            switch (response.status) {
                case 401:
                    ...
                    const isAuthEndpoint = requestUrl.includes('/Account/SignIn') || requestUrl.includes('/Account/SignUp') || requestUrl.includes('/Account/GoogleSignIn') || requestUrl.includes('/Account/FacebookSignIn');
                    if (isAuthEndpoint) {
                ...
                case 403:
                    {
                        const msg403 = response.data?.message || 'Bạn không có quyền truy cập chức năng này!';
                        toast.error(msg403);
                    }
                    break;
                case 404:
                    if (isUserAction) {
                        const msg404 = response.data?.message || 'Không tìm thấy tài nguyên yêu cầu!';
                        toast.error(msg404);
                    }
                    break;
                case 500:
                    {
                        const msg500 = response.data?.message || 'Có lỗi xảy ra trên server!';
                        toast.error(msg500);
                    }
                    break;
                default:
                    if (isUserAction) {
                        const msgDefault = response.data?.message || response.data?.Message || 'Có lỗi xảy ra!';
                        toast.error(msgDefault);
                    }
            }
        } else {
            const isUserAction = ['post', 'put', 'delete', 'patch'].includes(
